...
- **持久化会话**：追加式 JSONL 会记录消息、激活工具变化、检查点和压缩投影；会话可列出、
  恢复、分叉或回退。
- **理解仓库规范**：从仓库根目录到当前工作区分层加载受限大小的 `AGENTS.md`、
  `CLAUDE.md` 和 rules 目录，并支持可选的全局与工作区 Markdown 记忆。
...
`/attachments` 用于查看，`/detach [编号|all]` 用于移除。在任意 Token 边界输入 `@`
会打开仓库文件模糊搜索面板；继续输入可过滤，使用上下键选择，按 `Enter` 或 `Tab` 插入。
包含空格的路径会自动加引号。消息发送时，`修复 @src/parser.rs` 这样的引用会转成有大小
上限的附件；普通邮箱和不存在的 `@名称` 仍保持原文本。文件索引在 UI 线程之外运行，
遵循 Git Ignore、包含隐藏项目文件，并设置硬上限，避免大型仓库拖慢启动。
非交互任务也支持附件：
